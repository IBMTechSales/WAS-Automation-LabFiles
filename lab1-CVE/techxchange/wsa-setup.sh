...
echo "Login to OCP"
echo "" 
echo "oc login --username=ocadmin --password=ibmrhocp"
echo ""
echo "-----------------------------------------------------------------------"
echo "" 

oc login --username=ocadmin --password=ibmrhocp
...
sshpass -p "engageibm" ssh-copy-id -i ~/.ssh/wsa ibmuser@student.ocp.ibm.edu
#ssh-copy-id -i ~/.ssh/wsa ibmuser@student.ocp.ibm.edu
...
echo ""
echo "ssh-keyscan student.ocp.ibm.edu >> /home/ibmuser/wsa_known_hosts"
echo ""
echo "----------------------------------------------------------------------"

ssh-keyscan student.ocp.ibm.edu >> /home/ibmuser/wsa_known_hosts
...
echo""
echo "oc rsh $MANAGER_POD runcli testConnection student.ocp.ibm.edu linux"
echo ""
echo "-----------------------------------------------------------------------------------------------"


MANAGER_POD=$(oc get pod -l app.kubernetes.io/component=runbook-manager -o name | head -n 1)
oc rsh $MANAGER_POD runcli testConnection student.ocp.ibm.edu linux
